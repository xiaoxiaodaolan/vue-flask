  ...
  <div id="all">
	  ...
	  <table class="table2">
	  	<td>
			...
			<p><el-button type="primary" round @click="addbook()">确定</el-button></p>
...
<script>
	export default {
		...
		methods:{
			...
			addbook(){
				if(this.name.length != 0 && this.author.length !=0 && this.location.length !=0 && this.price.length !=0){
					var newbook = new URLSearchParams();
					newbook.append('name', this.name);
					newbook.append('author', this.author);
					newbook.append('location', this.location);
					newbook.append('price', this.price);
					this.$http.post('http://127.0.0.1:9000/addbook',newbook)
					.then(res => {
						newbook.name = this.name;
						newbook.author = this.author;
						newbook.location = this.location;
