  ...
  <div id="all">
	  	...
	  	<table class="table1">
	  		...
	  		<tr v-for="(book,index) in books">
				...
				<td>{{book.location}}</td>
	  			<td>{{book.price}}</td>
				<td>
					<el-button type="primary" icon="el-icon-edit" circle @click="updatebook(book)"></el-button>
				</td>
	  		</tr>
	  		<tr>
	  ...
	  <table class="table2">
	  	<td>
			...
			<p>位置:<input type="text" v-model="location" /></p>
	  		<p>价格:<input type="number" v-model="price" /></p>
			<p><el-button type="primary" round @click="updbook()">确定</el-button></p>
	  	</td>
	  </table>
	  </div>
  </div>
	...
	export default {
		...
		methods:{
			...
			updatebook(book) {
				...
				this.location = this.updatedBook.location;
				this.price = this.updatedBook.price;
				this.flag = true;
			},
			updbook() {
				var updateBook = new URLSearchParams();
				updateBook.append('id',this.id);
				updateBook.append('name',this.name);
				updateBook.append('author',this.author);
				updateBook.append('location',this.location);
				updateBook.append('price',this.price);
				this.$http.post('http://127.0.0.1:9000/updatebook',updateBook)
				.then(res => {					
					this.updatedBook.name = this.name;
					this.updatedBook.author = this.author;
					this.updatedBook.location = this.location;
