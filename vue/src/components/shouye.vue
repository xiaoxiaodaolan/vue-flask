<template>
	<div id="all">
				...
				<div id="nav">
						...
						<div v-for="fourmenu in fourmenus" @click='fourmenu.show=!fourmenu.show'>
							...
							<div v-show="fourmenu.show">
								<div>
									<!-- <button class="btn"><router-link to="/addgly"><span>添加教师</span></router-link></button> -->
		...
		<br />
		<br />
		<h1 class="sh1">欢迎进入图书管理系统|管理员</h1>
		<br />
		<table class="table3">
			<th colspan="8">借阅申请批准</th>
			<tr>
				<td>序号</td>
				<td>姓名</td>
				<td>学号</td>
			...
			<tr v-for="(user,index) in users">
				...
				<td>{{user.approve}}</td>
				<td>
					<button for="agreee" @mouseup="updateuser(user)" @click="upduser()">同意</button> | 
					<button for="agree" @mouseup="updateuser2(user)" @click="upduser()">拒绝</button>
				</td>
				<td>
					<el-button type="danger" icon="el-icon-delete" circle @mouseup.native="adddel(user)" @click="deluser(index)"></el-button>
				</td>
...
<script>
	export default {
		...
		methods:{
			...
			getuserlist(){
				this.$http.get('http://127.0.0.1:9000/userlist')
				.then(res => {
			...
			updateuser(user) {
				...
				this.id = this.updatedUser.id;
				this.stuname = this.updatedUser.stuname;
				this.sno = this.updatedUser.sno;
				this.bbook = this.updatedUser.bbook;
				this.timeborrow = this.updatedUser.timeborrow;
				this.approve = "已批准";
				this.flag = true;
			},
			updateuser2(user) {
			...
			upduser() {
				var updateUser = new URLSearchParams();
				updateUser.append('id',this.id);
				updateUser.append('stuname',this.stuname);
				updateUser.append('sno',this.sno);
				updateUser.append('bbook',this.bbook);
				updateUser.append('timeborrow',this.timeborrow);
				updateUser.append('approve',this.approve);
				this.$http.post('http://127.0.0.1:9000/updateuser',updateUser)
				.then(res => {
					this.updatedUser.stuname = this.stuname;
					this.updatedUser.sno = this.sno;
					this.updatedUser.bbook = this.bbook;
					this.updatedUser.timeborrow = this.timeborrow;
					this.updatedUser.approve = this.approve;					
					this.flag = false;
				})
			},
			adddel(user) {
			...
			deluser(){
				if(window.confirm('确认要删除？')){
					if(this.id.length != 0){
						...
						this.$http.post('http://127.0.0.1:9000/deleteuser',shanchu)
						.then(res => {
							this.getuserlist()
						})
					}else{
