<template>
  <div id="all">
	  ...
	  <div id="div1">
		...
		<el-input class="el-input" @click.native="againclick()" placeholder="请输入作者" v-model="author"></el-input>
		<el-button type="primary" icon="el-icon-search" @click="querybk()">搜索</el-button>
	  	<table class="table1">
	  		<tr>
	  			<td>序号</td>
	  			<td>书名</td>
...
<script>
	...
	export default {
		...
		methods:{
			...
			querybk(){
				var querybk = new URLSearchParams();
				if(this.author.length != 0){
					querybk.append('author',this.author);
					this.$http.post('http://127.0.0.1:9000/querybook',querybk)
					.then(res => {
						this.books = res.data.res;
						this.author = "";
					})
