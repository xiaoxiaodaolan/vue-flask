	  ...
	  <div id="div1">
	  	<table class="table1">
	  		...
	  		<tr v-for="(student,index) in students">
	  			...
	  			<td>
					<el-button type="danger" icon="el-icon-delete" circle @mouseup.native="adddel(student)" @click="delstudent(index)"></el-button>
	  			</td>
	  		</tr>
	  		<tr>
		...
		<table class="table2">
			<td>
				<p>删除账号</p>
				<p>学号:<input type="text" v-model="username" /></p>
				<p><el-button type="primary" round @click="delstudent(index)">确定</el-button></p>
...
<script>
	export default{
		...
		methods:{
			...
			getstudentlist(){
				this.$http.get('http://127.0.0.1:9001/yonghulist')
				.then(res => {
			...
			adddel(student) {
				...
				this.username = this.updatedStudent.username;
			},
			delstudent(){
				if(window.confirm('确认要删除？')){
					if(this.username.length != 0){
						var shanchu = new URLSearchParams();
						shanchu.append('username',this.username);
						this.$http.post('http://127.0.0.1:9001/deleteyonghupwd',shanchu)
						.then(res => {
							this.getstudentlist()
						})
					}else{
