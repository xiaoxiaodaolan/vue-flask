<template>
  <div id="all">
	  ...
	  <div id="div1">
		  ...
		  <el-button type="primary" icon="el-icon-search" @click="querystudent()">搜索</el-button>
		  <el-input class="el-input" @click.native="againclick()" placeholder="请输入学号" v-model="sno"></el-input>
		  <el-button type="primary" icon="el-icon-search" @click="querystu()">搜索</el-button>
	  	<table class="table1">
		...
		methods:{
			...
			querystudent(){
				...
				if(this.stuname.length != 0){
					querystudent.append('stuname',this.stuname);
			...
			querystu(){
				var querystu = new URLSearchParams();
				if(this.sno.length != 0){
					querystu.append('sno',this.sno);
					this.$http.post('http://127.0.0.1:9000/querystudent',querystu)
					.then(res => {
						this.students = res.data.res;
						this.sno = "";
					})
