					...
					<div v-for="twomenu in twomenus" @click='twomenu.show=!twomenu.show'>
						...
						<div v-show="twomenu.show">
							<div>
								...
								<button class="btn"><router-link to="/updatebook"><span>删除图书</span></router-link></button>
							</div>
						</div>
					</div>
					<div v-for="threemenu in threemenus" @click='threemenu.show=!threemenu.show'>
						...
						<div v-show="threemenu.show">
							<div>
								...
								<button class="btn"><router-link to="/delstupwd"><span>修改账号</span></router-link></button>
							</div>
						</div>
					</div>
		...
		<div id="div1">
			...
			<table class="table2">
				<td>
					...
					<!-- <p>归还日期:<input type="text" v-model="timereturn" /></p> -->
					<!-- <p>是否归还:<input type="text" v-model="statu" /></p> -->
					<p><el-button type="primary" round @click="addstu()">确定</el-button></p>
...
<script>
	export default {
		...
		methods:{
			...
			addstu(){
				if(this.stuname.length != 0 && this.sno.length !=0 && this.bbook.length !=0 && this.timeborrow.length !=0 ){
					var newborrow = new URLSearchParams();
					newborrow.append('stuname', this.stuname);
					newborrow.append('sno', this.sno);
					newborrow.append('bbook', this.bbook);
					newborrow.append('timeborrow', this.timeborrow);
					newborrow.append('timereturn', this.timereturn);
					newborrow.append('statu', this.statu);
					this.$http.post('http://127.0.0.1:9000/addborrow',newborrow)
					.then(res => {
						newborrow.stuname = this.stuname;
						newborrow.sno = this.sno;
						newborrow.bbook = this.bbook;
						newborrow.timeborrow = this.timeborrow;
						newborrow.timereturn = this.timereturn;
						newborrow.statu = this.statu;
						this.stus.push(newborrow);
						// this.stuname="";
						// this.sno="";
						// this.bbook="";
						// this.timeborrow="";
