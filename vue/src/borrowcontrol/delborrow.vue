	  ...
	  <div id="div1">
	  	<table class="table1">
	  		...
	  		<tr v-for="(stu,index) in stus">
				...
				<td>{{stu.timereturn}}</td>
	  			<td>{{stu.statu}}</td>
				<td>
					<el-button type="danger" icon="el-icon-delete" circle @mouseup.native="adddel(stu)" @click="delabook(index)"></el-button>
				</td>
	  		</tr>
	  		<tr>
	  	...
	  	<table class="table2">
	  		<td>
				...
				<p><el-button type="primary" round @click="delbook(index)">确定</el-button></p>
	  		</td>
	  	</table>
	  </div>
			...
			delbook(){
				if(window.confirm('确认要删除？')){
					if(this.stuname.length != 0){
						...
						shanchu.append('stuname',this.stuname);
						shanchu.append('bbook',this.bbook);
						this.$http.post('http://127.0.0.1:9000/deleteborrow',shanchu)
						.then(res => {
							this.getborrowlist()
						})
					}else{
			...
			delabook(){
				if(window.confirm('确认要删除？')){
					if(this.id.length != 0){
						...
						shanchu.append('bbook',this.bbook);
						this.$http.post('http://127.0.0.1:9000/deleteaborrow',shanchu)
						.then(res => {
							this.getborrowlist()
						})
					}else{
