<template>
  <div id="all">
	  ...
	  <div id="div1">
		  <el-input class="el-input" @click.native="againclick()" placeholder="请输入姓名" v-model="glyname"></el-input>
		  <el-button type="primary" icon="el-icon-search" @click="querygly()">搜索</el-button>
		  <el-input class="el-input" @click.native="againclick()" placeholder="请输入教师编号" v-model="tno"></el-input>
	...
	export default{
		...
		methods:{
			...
			querygly(){
				...
				if(this.glyname.length != 0){
					querygly.append('glyname',this.glyname);
			...
			queryadmin(){
				...
				if(this.tno.length != 0){
					queryadmin.append('tno',this.tno);
					this.$http.post('http://127.0.0.1:9003/querygly',queryadmin)
					.then(res => {
						this.glys = res.data.res;
						this.tno = "";
					})
