  ...
  <div id="all">
	  ...
	  <table class="table2">
	  	<td>
	  		...
	  		<p>教师编号:<input type="number" v-model="tno" /></p>
	  		<p>联系电话:<input type="number" v-model="tel" /></p>
			<p><el-button type="primary" round @click="addgly()">确定</el-button></p>
...
<script>
	export default{
		...
		methods:{
			...
			addgly(){
				if(this.glyname.length != 0 && this.tno.length !=0 && this.tel.length !=0){
					var newgly = new URLSearchParams();
					newgly.append('glyname', this.glyname);
					newgly.append('tno', this.tno);
					newgly.append('tel', this.tel);
					this.$http.post('http://127.0.0.1:9003/addgly',newgly)
					.then(res => {
						newgly.glyname = this.glyname;
						newgly.tno = this.tno;
						newgly.tel = this.tel;						
						this.glys.push(newgly);
						this.glyname="";
						this.tno="";
						this.tel="";
					})
