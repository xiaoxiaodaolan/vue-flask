	  ...
	  <div id="div1">
	  	<table class="table1">
	  		...
	  		<tr v-for="(gly,index) in glys">
				...
				<td>{{gly.tel}}</td>
	  			<td>
					<el-button type="primary" icon="el-icon-edit" circle @click="updategly(gly)"></el-button>
	  			</td>
	  		</tr>
	  		<tr>
		...
		<table class="table2">
			<td>
				<p>修改学生信息</p>
				<p>姓&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;名:<input type="text" v-model="glyname" /></p>
				<p>教师编号:<input type="text" v-model="tno" /></p>
				<p>联系电话:<input type="number" v-model="tel" /></p>
				<p><el-button type="primary" round @click="updgly()">确定</el-button></p>
			</td>
		</table>
	  </div>
  </div>
...
<script>
	export default{
		...
		methods:{
			...
			updategly(gly) {
				this.updatedgly = gly;
				this.id = this.updatedgly.id;
				this.glyname = this.updatedgly.glyname;
				this.tno = this.updatedgly.tno;
				this.tel = this.updatedgly.tel;
				this.flag = true;
			},
			updgly() {
				var updategly = new URLSearchParams();
				updategly.append('id',this.id);
				updategly.append('glyname',this.glyname);
				updategly.append('tno',this.tno);
				updategly.append('tel',this.tel);
				this.$http.post('http://127.0.0.1:9003/updategly',updategly)
				.then(res => {	
					this.updatedgly.glyname = this.glyname;
					this.updatedgly.tno = this.tno;
					this.updatedgly.tel = this.tel;
